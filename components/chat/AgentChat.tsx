    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          className={`${chatPanelMobile} ${chatPanelDesktop} rounded-2xl border border-accent/25 bg-[#05070d]/95 shadow-[0_0_60px_rgba(101,227,255,0.15)] backdrop-blur-xl sm:right-[6.5rem]`}
        >
          ...
          <div ref={listRef} className={chatMessagesArea}>
            {messages.map((msg) => (
              <div
                key={msg.id}
...
export function AgentChatButton({
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.95 }}
      className={`${chatFabBase} ${chatFabBottom} right-[4.75rem] border border-accent/40 bg-[#05070d]/90 shadow-[0_0_40px_rgba(101,227,255,0.25)] hover:shadow-[0_0_50px_rgba(101,227,255,0.4)] sm:right-[6.5rem]`}
      aria-label={open ? "Zatvori AI chat" : "Otvori ČOKOLADNI AGENT chat"}
      aria-expanded={open}
    >
      <AnimatePresence mode="wait">
        {open ? (
          ...
          <motion.div
            ...
            className="relative"
          >
            <Logo size="sm" className="h-9 w-9 rounded-full" />
          </motion.div>
        )}
      </AnimatePresence>
    </motion.button>
